...
if ( isset($_POST['sign']) ) {
  ...
  $query = "SELECT lic, email FROM driver WHERE lic='$lic'";
    $result = mysqli_query($conn,$query);
    $row=mysqli_fetch_assoc($result);
    $count = mysqli_num_rows($result);
    if($count==0){
      $error = true;
      $errMSG = "Provided license number does not exist.";
    }else{
      $email = $row['email'];
    }
  ...
  $query = "SELECT username FROM login WHERE email='$email'";
    ...
    if($count!=0){
      $error = true;
      $errMSG = "Provided license number is already associated with an account.";
    }
  }

  if(!$error ) {
    $query = "INSERT INTO login(username, email, password) VALUES('$usr', '$email', '$password')";
...
<html>
<head>
  <link rel="stylesheet" type="text/css" href="..\CSS\admin_per.css">
...
<body>
  ...
  <div class="loginBox">
      ...
      <form method="post" action="<?php $_SERVER['PHP_SELF']?>">
        <p>LICENSE NUMBER </p>
        <input class="inp" type="text" name="lic" placeholder="Enter Licence No">
        <p >USERNAME </p>
